...
> 이 문서는 **참조용**이다. 여기 적힌 패턴에 해당하지 않는 문제도 에이전트는
> 빌드/실행 로그와 상태 정보, `API_SPEC.md`, KB를 바탕으로 추론하여 처리할 수 있다.
...
### 이미지 / 빌드 산출물 5GB 초과 (known limit)
- Cloudtype Hobby 기준 이미지 5GB 제한.
- 우선 시도:
  - 더 가벼운 base image, 멀티스테이지 빌드.
  - `.dockerignore` / 불필요 파일 제외.
  - 정적 산출물 경로/번들 분할.
- 운영자 자동 문의 대상 아님(우선은 빌드/이미지 최적화로 처리).
...
> 운영자는 사용자의 소스코드에 접근하지 않는다. 문의로 유도할 때는
> **배포 설정/로그 관점**의 정보만 정리해 안내한다.
