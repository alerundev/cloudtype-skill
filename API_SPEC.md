...
- **자동 리소스 조정** — OOM 또는 응답 지연이 감지되더라도 보고만 수행하며, cpu/memory/disk/replicas/spot 을 자동으로 변경하지 않습니다. 이유: 구독 풀 제약, 다른 서비스 영향 우려, 사용자의 운영 판단 보존. (사용자가 "프리티어/무료/테스트" 등을 명시한 경우는 자동 조정이 아닌 **명시 의도**이므로, [Resources / `spot` 시맨틱](#resources--spot-시맨틱) 을 따릅니다.)
...
| `resources.spot` | 의미 | 특성 |
|---|---|---|
| `true` | **프리티어 리소스** | 카드 등록 시 부여되는 1GB 무료 풀. 매일 1회 자동 정지, 불특정 시점 다운타임. 테스트용. |
| `false` | **구독 리소스** | Hobby / Pro 구독 풀에서 차감. 고가용. 운영용. |
| (키 자체 생략) | 서버 디폴트 | 호출자 계정 상황에 따라 분기 — 임의로 가정하지 않습니다. |

> ⚠️ **이름으로 의미를 추측하지 마세요.** AWS 의 spot 인스턴스(저렴 수시가능)와는 **완전히 다른 개념**입니다. Cloudtype 에서는 `spot: true` 가 프리티어(무료), `spot: false` 가 구독(유료)입니다.

**구독 사용자도 `spot: true` 를 사용할 수 있습니다.** 구독 풀이 모자란 상태에서 임시 테스트만 해보고 싶을 때 등을 위해, 구독 플랜이어도 프리티어 풀(1GB)을 함께 활용할 수 있도록 설계되어 있습니다.

#### 사용자 의도 → `spot` 매핑

사용자가 원하는 리소스 종류를 말로 명시한 경우에는 **그 의도를 PUT 페이로드에 명시적으로 반영**합니다. 이는 "리소스 자동 조정" 이 아니라 사용자 명시 의도 처리입니다.

- "프리티어 / free tier / 무료 / 테스트용 / 임시로" → `resources: {spot: true}`
- "구독 리소스 / 운영용 / 프로덕션 / 무중단" → `resources: {spot: false}`
- 어느 쪽도 명시 없음 → `resources` 자체 생략

사용자가 프리티어를 명시했다면 cpu/memory/disk 같은 다른 필드를 임의로 채우지 않습니다. `spot: true` 만 보내면 서버가 프리티어 풀에 맞는 디폴트로 채웁니다.

#### 계정별 디폴트 검증 상태 (TODO)

`resources` 키를 완전히 생략한 상태에서 서버가 어떤 디폴트를 적용하는지는 **계정 상황(프리티어 플랜 / Hobby / Pro)에 따라 다르며, 현재 모두 검증되지 않았습니다.** 임의로 가정하지 않고, 사용자 의도가 명확한 경우에는 해당 값을 명시하고 그렇지 않으면 키를 생략합니다.

자세한 제약과 의도 매핑은 `reference/free-tier.md` 참조.
...
- 특히, 사용자 명시가 없는 상태에서 **`spot: false` 같은 값을 디폴트로 가정하여 넣지 않습니다.** 서버가 호출자 계정 상황에 맞게 적절한 디폴트를 적용합니다.
