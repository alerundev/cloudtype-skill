...
## 🎟️ 프리티어 vs 구독 리소스 (`resources.spot`)

| `resources.spot` | 의미 |
|---|---|
| `true` | **프리티어** — 무료 1GB 풀, 테스트용, 매일 1회 정지 |
| `false` | **구독** — Hobby/Pro 풀, 운영용 |
| (생략) | 서버 디폴트 — 임의로 `false` 등을 채우지 않습니다 |

> ⚠️ AWS 의 spot 인스턴스와는 무관. 이름으로 의미를 추측하지 마세요.

구독 사용자도 `spot: true` 를 사용할 수 있습니다 (프리티어 풀 공용).

**의도 처리**
- 사용자가 프리티어/무료/테스트 쪽을 원한다고 말하면 → `resources.spot: true` 명시
- 구독/운영/무중단 쪽을 원한다고 말하면 → `resources.spot: false` 명시
- 어느 쪽도 명시 없으면 → `resources` 자체 생략

프리티어를 명시한 경우에도 cpu/memory/disk 같은 다른 필드는 임의로 채우지 않습니다. `spot: true` 만 보내면 서버가 프리티어 풀에 맞게 디폴트를 채웁니다.

**완료 보고 시 검증**
- 응답의 `resources.spot` 을 "프리티어 리소스" / "구독 리소스" 로 변환해 명시합니다 (키가 없으면 `GET .../deployment/<name>` 으로 재조회).
- 사용자 의도와 다르면 완료로 보고하지 말고 즉시 알린 뒤 결정을 기다립니다.

프리티어 제약/한도, 의도 표현 매핑, 플랜 전환 주의 등 세부는 `reference/free-tier.md` 참조.
...
본 문서에서 다루지 않는 세부 사항은 다음을 참조합니다.

- **API 전체 명세**: `API_SPEC.md`
- **상태 머신**: `reference/state-machine.md`
- **빌드/실행 오류 패턴 (보조 자료)**: `reference/diagnose-patterns.md`
- **프리티어 리소스 상세**: `reference/free-tier.md`
