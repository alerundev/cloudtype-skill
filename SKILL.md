...
사용자가 프리티어/무료/테스트 쪽을 명시하면 `spot: true`, 구독/운영 쪽을 명시하면 `spot: false`, 아니면 `resources` 자체를 생략합니다.

배포 완료 보고 시 응답의 `resources.spot` 을 "프리티어 리소스" / "구독 리소스" 로 변환해 명시하고, 사용자 의도와 다르면 완료 보고를 보류하고 보고합니다.

의미의 배경, 의도 표현 매핑, 프리티어 제약/한도, 플랜 전환 주의 등 세부는 `reference/free-tier.md` 참조.
