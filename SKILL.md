...
`resources` 페이로드에는 풀 종류만 명시합니다. `cpu` / `memory` / `disk` 같은 세부 사양은 사용자가 명시한 경우에만 포함합니다. 사용자 명시가 없는 상태에서 임의로 값을 잔으면 Cloudtype 이 preset 별로 이미 설계해둔 리소스 배분 폴리시를 깨버리며, 특히 프리티어 풀에서는 1GB 내에 DB+백엔드+프론트 풀세트가 들어갈 수 있는 여유를 남겨둔 디폴트 배분을 깨버려 후속 배포를 막을 수 있습니다. "서비스당 최소치는 이 정도겠지" 의 LLM 상식 추측 금지.
...
**자동 선택 결과가 프리티어 풀인 경우** (사용자가 풀을 명시하지 않았는데 잔여량 기반으로 프리티어 풀이 선택된 경우) 는 완료 보고에 프리티어 풀의 운영상 특성을 함께 안내합니다 — 주기적 자동 중지, 다운타임 가능성, 운영 용도면 구독 풀 권장 등. 사용자가 직접 프리티어를 명시한 경우는 이미 알고 있는 것으로 보고 안내를 생략합니다.
...
후보가 마스터 목록에 없으면 사용자에게 알립니다 — "Cloudtype 이 X 를 지원하지 않습니다." 임의로 가까운 다른 preset 으로 갈아타지 마세요. 호환 가능한 대체 후보가 있다면 (예: `mysql` 요청 → `mariadb` 호환) **제안하고 사용자 확인을 받은 뒤** 진행합니다.

preset 추론은 **최초 배포 시점 한 번** 만 사용합니다. 실패 후 다른 preset 으로 갈아타는 근거로 사용하지 않습니다.

### 추론 디폴트 (사용자 명시가 있으면 그게 절대 우선)

사용자가 다음 항목을 명시하지 **않은 경우에만** 아래 디폴트를 적용합니다. 명시가 있으면 (예: "mysql 로 해줘", "python flask 백엔드") 그 선택이 **절대 우선**이며, 디폴트로 덮어쓰지 않습니다.
...
이 디폴트는 마찰 감소용입니다 — 개발 지식이 없는 사용자의 "풀스택 서비스 하나 만들어줘" 같은 막연한 요청에서 매번 "어떤 DB요? 어떤 언어요?" 를 묻지 않고 한 번에 진행하기 위해서입니다. **디폴트를 적용했다면 완료 보고에 반드시 명시**합니다 (예: "DB 종류 디폴트로 PostgreSQL 을 사용했습니다. 다른 걸 원하시면 알려주세요").

⚠️ **사용자가 catalog 에 없는 것을 명시한 경우** (예: "mysql 로 해줘" → catalog 에는 `mariadb` 만 있음) 는 위 디폴트 규칙이 아니라 "호환 대체 제안 + 사용자 확인" 경로로 이동합니다. 사용자 명시를 조용히 PostgreSQL/Node 로 바꾸지 마세요.
